...
export async function getProfileModel(userId: string, actorRole: string) {
  await query("BEGIN");

  try {
    // Fetch profile, excluding sensitive fields
    const profile = await query(
      `SELECT id, role, email, phone_number, display_name, country, timezone, is_verified, created_at, updated_at
       FROM users
       WHERE id=? LIMIT 1`,
      [userId]
    );

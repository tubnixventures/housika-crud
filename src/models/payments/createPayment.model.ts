...
export async function createPaymentModel(payment: PaymentType) {
  const parsed = PaymentSchema.parse(payment);
  return await query(
    `INSERT INTO payments (id, user_id, booking_id, property_id, room_number, receipt_id, receipt_number, reference_type, gateway, gateway_reference, amount, currency, status, created_at, updated_at)

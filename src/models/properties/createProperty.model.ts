...
export async function createPropertyModel(property: PropertyType) {
  const parsed = PropertySchema.parse(property);
  return await query(
    `INSERT INTO properties (id, landlord_id, units_available, active_units, occupied_units, video_urls, image_urls, description, country, currency, location, exact_location, created_at, updated_at)

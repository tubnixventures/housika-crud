...
export async function deleteReceiptController(req: any) {
  ...
  const res = await fetch(`${storageUrl}/delete?bucket=receipts`, {
    method: "DELETE",
    headers: {
  ...
  if (!res.ok) {
    throw new Error(storageResult.error || `Failed to delete receipt files: ${res.statusText}`);
  }

  // Only delete DB record if storage deletion succeeded
  const dbResult = await deleteReceiptModel(req.params.id);

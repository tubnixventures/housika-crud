...
export async function updateContactMessageModel(id, updates) {
    const parsed = ContactMessageUpdateSchema.parse(updates);
    const fields = Object.keys(parsed).map((k) => `${k}=?`).join(", ");
    const values = Object.values(parsed);
    return await query(`UPDATE contact_messages SET ${fields}, updated_at=CURRENT_TIMESTAMP WHERE id=?`, [...values, id]);
}

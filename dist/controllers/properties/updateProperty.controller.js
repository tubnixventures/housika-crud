...
export async function updatePropertyController(req) {
    ...
    try {
        ...
        if (Array.isArray(req.body.rooms)) {
            ...
            const incomingRoomNumbers = req.body.rooms.map((r) => r.room_number);
            ...
            for (const room of req.body.rooms) {
                ...
                const match = existingRooms.find((r) => r.room_number === room.room_number);
                if (match) {
                    // Update existing room
                    await query(`UPDATE rooms SET category=?, currency=?, country=?, location=?, exact_location=?, amount=?, period=?, capacity=?, amenities=?, image_urls=?, video_urls=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`, [
                        room.category,
                        room.currency,
                        room.country,
                        room.location,
                    ...
                    // Insert new room
                    const roomId = uuidv4();
                    await query(`INSERT INTO rooms (id, property_id, room_number, category, currency, country, location, exact_location, amount, period, is_occupied, capacity, amenities, image_urls, video_urls, created_at, updated_at)

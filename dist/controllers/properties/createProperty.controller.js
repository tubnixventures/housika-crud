...
export async function createPropertyController(req) {
    ...
    if (payload.role !== "admin") {
        const paymentResult = await query(`SELECT * FROM payments WHERE id = ? AND status = 'success'`, [req.body.payment_id]);
    ...
    try {
        ...
        if (Array.isArray(req.body.rooms)) {
            for (const room of req.body.rooms) {
                const roomId = uuidv4();
                await query(`INSERT INTO rooms (id, property_id, room_number, category, currency, country, location, exact_location, amount, period, is_occupied, start_at, ends_at, image_urls, video_urls, capacity, amenities, created_at, updated_at)
        ...
        if (payload.role !== "admin" && payment) {
            ...
            await query(`INSERT INTO receipts (id, receipt_number, user_id, landlord_id, property_id, payment_id, amount, currency, verification_status, issued_by, created_at, updated_at)
